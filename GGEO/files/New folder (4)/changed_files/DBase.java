package com.theorem.misc.dbase;
...
public class DBase
{
	...
	*/
	...
	// This is not part of DBase, but a problem in cross internationalization.
	// We will fix incorrect decimal separators with the correct one for the current localization.
	char correctDecimalSeparator;	
	char wrongDecimalSeparator;
	...
	* @param locale Locale for decimal symbol.
	* @throws SQLException if the default memo handler can't be found.
	*/
	public DBase(String path, Locale locale) throws SQLException
	{
		dbPath = path;
		memoHandler = MemoHandler.getInstance("DBASEIVMemoHandler");
		memoHandler.setBlockSize(Record.BLOCKSIZE);
		
		// Pick this stuff up as an aid to handling floating point numbers.
		// We can try to make the correction if the symbols were entered in a 
		// different locale.
		...
		correctDecimalSeparator = dfs.getDecimalSeparator();
		wrongDecimalSeparator = correctDecimalSeparator == '.' ? ',' : '.';
	}

	/**
	* Constructor to handle a DBase directory with default system locale.
	*
	* @param path Path to the DBase directory holding dbase tables.
	* @throws SQLException if the default memo handler can't be found.
	*/
...
//	}
	...
	*/
	public String getMemo(String name) throws SQLException
	{
		...
		if (currentMemoName != null && currentMemoName.equals(pName))
			if (memo != null)
	...
	*/
	public int findColumn(String name) throws SQLException
	{
		...
		if (i >= fName.length)
		{
			...
			throw new SQLException("Can't find field name " + name);
		}
	...
	class Record
	{
		...
		*/
		public double getNumberData(int fdidx) throws SQLException
		{
			...
			// Sometimes floating point data will be in mixed format, European and English.
			// Determine the localization and fix things for that system.
			number = number.replace(wrongDecimalSeparator, correctDecimalSeparator);
